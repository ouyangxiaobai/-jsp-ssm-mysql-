...
<script type="text/javascript">
	function change(index,size,username) {
		location.href="findLogtable?index="+index+"&size="+size+"&username="+username;
	}
</script>
</head>
	...
	<form action="findLogtable" method="post">
		...
		<table>
			...
			<c:forEach items="${mpb.list}" var="logtable">
				<tr>
					<td>${logtable.logid }</td>
					<td>${logtable.username }</td>
					<td>${logtable.action }</td>
					<td>${logtable.actiontime }</td>
				</tr>
			</c:forEach>
			<tr>
